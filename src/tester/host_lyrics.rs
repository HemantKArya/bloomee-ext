    ...
    fn http_request(
        ...
        match req.send() {
            Ok(resp) => {
                ...
                Ok(component::lyrics_provider::utils::HttpResponse {
                    status,
                    headers,
                    body,
                })
    ...
    loop {
        ...
        match choice {
            "1. Get lyrics by track ID" => {
                ...
                match bindings
                    .component_lyrics_provider_lyrics_api()
                    ...
                    .call_search(&mut store, &q)?
                {
                    Ok(results) => {
                        ...
                        if let Some(r) = results
                            .iter()
                            .find(|r| sel.ends_with(&format!("[{}]", r.id)))
                        {
                            match bindings
                                .component_lyrics_provider_lyrics_api()
...
fn print_full_lyrics(
    ...
    if let Some(auth) = &meta.author {
        println!("  author    : {auth}");
    }
    if let Some(lang) = &meta.language {
        println!("  language  : {lang}");
    }
    if let Some(copy) = &meta.copyright {
        println!("  copyright : {copy}");
    }
    ...
    println!(
        "  instrumental: {}",
        if lyrics.is_instrumental { "yes" } else { "no" }
    );
